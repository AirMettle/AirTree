...
inline uint64_t countPreciseBins2DxP(const TLEoption3_2D *root) {
  if (!root) {
    return 0;
  }
...
protected:
  ...
  template <typename T> void runCreateAndInsert(::benchmark::State &state) {
    ...
    const uint64_t precise_bins = countPreciseBins2DxP(airTree2DxP_root.get());
...
#endif // AIRTREE_BENCH_GENERATE_TRIE2D_2DXP_HPP
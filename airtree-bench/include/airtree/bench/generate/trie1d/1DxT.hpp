...
#ifndef AIRTREE_BENCH_GENERATE_TRIE1D_1DXT_HPP
...
inline uint64_t countPreciseBins1DxT(const TrieNode_13 *root) {
  if (!root) {
    return 0;
  }
  ...
  for (unsigned int i = 0; i < BINS_256; ++i) {
    ...
    for (unsigned int j = 0; j < BINS_32; ++j) {
...
class AirTreeBench1DxT : public benchmark::Fixture {
protected:
  std::unique_ptr<SpecialCounts> specialCounts;
  uint64_t curr_trie_size = 0;

  std::unique_ptr<TrieNode_13> airTree1DxT_root;
  ...
  template <typename T> void runCreateAndInsert(::benchmark::State &state) {
    ...
    for (auto _ : state) {
      ...
      airTree1DxT_root = execCreateAndInsert_TrieNode13(
          *specialCounts, curr_trie_size, fpharray);

      benchmark::DoNotOptimize(airTree1DxT_root);
    }

    const uint64_t total_input_bytes =
        static_cast<uint64_t>(state.iterations()) * dataset_size_bytes;
    const uint64_t precise_bins = countPreciseBins1DxT(airTree1DxT_root.get());
  ...
  template <typename T> void runSerialize(benchmark::State &state) {
    ...
    for (auto _ : state) {
      auto serializedTrieLocal = execSerialization_TrieNode13(
          airTree1DxT_root, *specialCounts, curr_trie_size);
...
#endif // AIRTREE_BENCH_GENERATE_TRIE1D_1DXT_HPP
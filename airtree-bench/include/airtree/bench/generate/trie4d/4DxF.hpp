...
inline uint64_t countPreciseBins4DxF(const TLE_4D_4x8 *root) {
  if (!root) {
    return 0;
  }
  ...
  for (unsigned int i = 0; i < BINS_4096; ++i) {
    ...
    for (unsigned int j = 0; j < BINS_256; ++j) {
      ...
      for (unsigned int k = 0; k < BINS_256; ++k) {
        if (!root->nodes[i]->nodes[j]->populated.test(k) ||
            !root->nodes[i]->nodes[j]->nodes[k]) {
          continue;
        }
        precise_bins +=
            countPreciseBins1DxF(root->nodes[i]->nodes[j]->nodes[k].get());
      }
    }
  }
  return precise_bins;
...
class AirTreeBench4DxF : public benchmark::Fixture {
protected:
  std::unique_ptr<SpecialCounts> specialCounts;
  uint64_t curr_trie_size = 0;

  std::unique_ptr<TLE_4D_4x8> airTree4DxF_root;
  ...
  template <typename T> void runCreateAndInsert(::benchmark::State &state) {
    ...
    const uint64_t precise_bins = countPreciseBins4DxF(airTree4DxF_root.get());
  ...
  template <typename T> void runSerialize(benchmark::State &state) {
    ...
    for (auto _ : state) {
      auto serializedTrieLocal = execSerialize_4D_4x8(
          airTree4DxF_root.get(), curr_trie_size, specialCounts);
...
#endif // AIRTREE_BENCH_GENERATE_TRIE4D_4DXF_HPP
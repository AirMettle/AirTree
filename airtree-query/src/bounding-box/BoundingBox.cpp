...
BoxCoordinate2DResultPair
BoundingBox::getCountsUnbounded(BoundingBoxCoordinate2D box) const {
  if (!histogram_) {
    throw std::runtime_error("Histogram not initialized.");
  }
  ...
  // Extract input min coordinates for each dimension
  double input_x_min = box.getMinX();
  double input_y_min = box.getMinY();

  // std::cout << "Input Box: (" << input_x_min << ", "
  //           << std::numeric_limits<double>::infinity() << "), (" <<
  //           input_y_min
  //           << ", " << std::numeric_limits<double>::infinity() << ")"
  //           << std::endl;
  // find the lowest bin that contains the min coordinates
  uint32_t bin_x_min = histogram_->getBinIndex(input_x_min);
  uint32_t bin_y_min = histogram_->getBinIndex(input_y_min);

  // get the reconstructed values for the xmin and ymin
  double in_recon_x_min = histogram_->getFPNumber(bin_x_min);
  double in_recon_y_min = histogram_->getFPNumber(bin_y_min);

  BoundingBoxCoordinate2D safe;
  BoundingBoxCoordinate2D edge;
  uint32_t safe_count = 0;
  uint32_t edge_count = 0;
  // Set the max values to infinity
  safe.setMaxX(std::numeric_limits<double>::infinity());
  safe.setMaxY(std::numeric_limits<double>::infinity());
  edge.setMaxX(std::numeric_limits<double>::infinity());
  edge.setMaxY(std::numeric_limits<double>::infinity());

  // lets setup the safe box coordinates
  if (input_x_min >= in_recon_x_min) {
    // We have a perfect safe xmin
    safe.setMinX(in_recon_x_min);
  } else {
    safe.setMinX(in_recon_x_min);
  }
  if (input_y_min >= in_recon_y_min) {
    // We have a perfect safe ymin
    safe.setMinY(in_recon_y_min);
  } else {
    safe.setMinY(in_recon_y_min);
  }
  ...
  if ((safe.getMinX() == input_x_min && safe.getMinY() == input_y_min)
      || histogram_->getBinIndex(safe.getMinX()) == 0
      || histogram_->getBinIndex(safe.getMinY()) == 0) {
    // If the safe box is exactly the input box or calculating the edge is out
    // of bounds then we can return early
    BoxCoordinate2DResult safe_box = {safe, safe_count};
    BoxCoordinate2DResult edge_box = {
        safe, 0}; // Edge box is same as safe - but we indicate no edge count
    return BoxCoordinate2DResultPair{safe_box, edge_box};
  }


  // at this point we should have a tight safe bounding box - now we can
  // calculate the edge bounding box
  // std::cout << "Safe Box: (" << safe.getMinX() << ", " << safe.getMaxX()
  //           << "), (" << safe.getMinY() << ", " << safe.getMaxY() << ")"
  //           << std::endl;
  // std::cout << "safe bitset: "
  //           << std::bitset<12>(histogram_->getFPNumber(
  //                  histogram_->getBinIndex(safe.getMinX())))
  //           << ", "
  //           << std::bitset<12>(histogram_->getFPNumber(
  //                  histogram_->getBinIndex(safe.getMinY())))
  //           << std::endl;
  edge.setMinX(histogram_->getBinLowerBound(
      histogram_->getBinIndex(safe.getMinX()) - 1));
  edge.setMinY(histogram_->getBinLowerBound(
      histogram_->getBinIndex(safe.getMinY()) - 1));
  // update the edge bin counts
  // get 12 bit internal representations for the edge box
  // std::cout << "Edge Box: (" << edge.getMinX() << ", " << edge.getMaxX()
  // << "), (" << edge.getMinY() << ", " << edge.getMaxY() << ")"
  // << std::endl;
  uint32_t edge_x_min_12 = histogram_->getInternalRepresentation(
      histogram_->getBinIndex(edge.getMinX()));
  uint32_t edge_y_min_12 = histogram_->getInternalRepresentation(
      histogram_->getBinIndex(edge.getMinY()));
  // std::cout << "edge x min 12: " << std::bitset<12>(edge_x_min_12)
  //           << ", edge y min 12: " << std::bitset<12>(edge_y_min_12)
  //           << std::endl;
  // capture tle
  uint32_t edge_x_min_tle = getTLEEncoding((edge_x_min_12 >> 10) & 0x3);
  uint32_t edge_y_min_tle = getTLEEncoding((edge_y_min_12 >> 10) & 0x3);
  // std::cout << "edge x min tle: " << std::bitset<2>(edge_x_min_tle)
  // << ", edge y min tle: " << std::bitset<2>(edge_y_min_tle)
  // << std::endl;
  uint32_t edge_x_min_10 = edge_x_min_12 & 0x3FF; // Get the last 10 bits
  uint32_t edge_y_min_10 = edge_y_min_12 & 0x3FF; // Get the last 10 bits
  uint64_t edge_internal_rep_min =
      combine_chunks_10b(edge_x_min_10, edge_y_min_10);
  uint32_t edge_combined_tle_min = (edge_x_min_tle << 3) | edge_y_min_tle;
  // get the counts for the edge box
  edge_count = getCount<TLEoption3_2D>(
      trie_root, edge_internal_rep_min, edge_combined_tle_min);

  auto safe_box = BoxCoordinate2DResult{safe, safe_count};
  auto edge_box = BoxCoordinate2DResult{edge, edge_count};
  // std::cout << "Safe Box: (" << safe.getMinX() << ", " << safe.getMaxX()
  //           << "), (" << safe.getMinY() << ", " << safe.getMaxY()

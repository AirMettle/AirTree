...
enum class SUPPORTED_FILE_TYPE { BINARY, PARQUET, /* ... */ };
enum class SUPPORTED_DATA_TYPE { INT32, INT64, FLOAT, DOUBLE, IGNORE };
...
dependencies. There is no `find_package(AirTree)` config (yet) — link
directly. Shipping a proper `AirTreeConfig.cmake` (and an `airtree.pc` for
`pkg-config`) that propagates these as `INTERFACE_LINK_LIBRARIES` on
imported targets is the long-term fix; tracked separately.
...
| `libmimalloc.a`            | airtree-core's allocator                     |
...
g++ -std=c++17 your_program.cpp \
  ...
  -lspdlog libmimalloc.a -lssl -lcrypto -lpthread -ldl -lrt -lm \
  -o your_program
```

...
std::vector<char> Merge1DxT::merge(const std::vector<char> &buffer1,
  ...
  SPDLOG_LOGGER_INFO(logger(), "Serialized merged trie (root) successfully");
  mergedRoot.reset();
...
Merge1DxT::mergeTrieNode13Level1(std::unique_ptr<TrieNode_13_Level1> node1,
  ...
  SPDLOG_LOGGER_INFO(logger(), "Entering mergeTrieNode13Level1");
  if (!node1) {
  ...
  for (size_t i = 0; i < BINS_32; ++i) {
    SPDLOG_LOGGER_INFO(logger(), "Merging Trie13 Level1 index {}: {} + {}", i,
...
mergeTrieNode13(std::unique_ptr<TrieNode_13> node1,
  ...
  SPDLOG_LOGGER_INFO(logger(), "Entering mergeTrieNode13");
  if (!node1) {
    SPDLOG_LOGGER_INFO(logger(), "node1 is nullptr, returning node2");
    return node2;
  }
  ...
  for (size_t i = 0; i < BINS_256; ++i) {
    SPDLOG_LOGGER_INFO(logger(), "Merging Trie13 Level0 index {}", i);

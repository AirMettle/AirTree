...
std::pair<TLE, unsigned int> internal_8bit(const FPHArray &array,
  ...
  case FPH_dtype::Double: {
    ...
    std::memcpy(&fpNumber1_64, &static_cast<const double *>(array.values)[i],
                sizeof(double));
    tle = setTLEComponents(fpNumber1_64);
    result = createInternal8Bit(fpNumber1_64, default_mode);
    break;
  }
  case FPH_dtype::Float: {
    double doubleValue =
        static_cast<double>(static_cast<const float *>(array.values)[i]);
    uint64_t fpNumber1_32;
    std::memcpy(&fpNumber1_32, &doubleValue, sizeof(doubleValue));
    tle = setTLEComponents(fpNumber1_32);
    result = createInternal8Bit(fpNumber1_32, default_mode);
    break;
  }
  case FPH_dtype::Int32: {
    double floatValue =
        int32_to_double(static_cast<const int32_t *>(array.values)[i]);
    uint64_t fpNumber1_32;
    std::memcpy(&fpNumber1_32, &floatValue, sizeof(float));
    tle = setTLEComponents(fpNumber1_32);
    result = createInternal8Bit(fpNumber1_32, default_mode);
    break;
  }
  case FPH_dtype::Int64: {
    double floatValue =
        int64_to_double(static_cast<const int64_t *>(array.values)[i]);
    uint64_t fpNumber1_32;
    std::memcpy(&fpNumber1_32, &floatValue, sizeof(float));
    tle = setTLEComponents(fpNumber1_32);
    result = createInternal8Bit(fpNumber1_32, default_mode);
  ...
  default:
    SPDLOG_LOGGER_ERROR(logger(), "Unknown data type for Dimension");
    break;
  }
...
std::pair<TLE, unsigned int> internal_10bit(const FPHArray &array,
  ...
  case FPH_dtype::Double: {
    ...
    std::memcpy(&fpNumber1_64, &static_cast<const double *>(array.values)[i],
                sizeof(double));
    tle = setTLEComponents(fpNumber1_64);
    result = createInternal10Bit(fpNumber1_64, default_mode);
    break;
  }
  case FPH_dtype::Float: {
    double doubleValue =
    ...
    std::memcpy(&fpNumber1_32, &doubleValue, sizeof(doubleValue));
    tle = setTLEComponents(fpNumber1_32);
    result = createInternal10Bit(fpNumber1_32, default_mode);
    break;
  }
  case FPH_dtype::Int32: {
    double floatValue =
        int32_to_double(static_cast<const int32_t *>(array.values)[i]);
    uint64_t fpNumber1_32;
    std::memcpy(&fpNumber1_32, &floatValue, sizeof(double));
    tle = setTLEComponents(fpNumber1_32);
    result = createInternal10Bit(fpNumber1_32, default_mode);
    break;
  }

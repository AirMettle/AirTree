...
// Mask words from a node's populated bitset.
template <std::size_t N>
inline void maskFromBitset(const std::bitset<N> &populated, uint64_t *words) {
  for (std::size_t w = 0; w < (N + 63) / 64; ++w) {
    const std::size_t base = w * 64;
    const std::size_t end = std::min(N, base + 64);
    uint64_t m = 0;
    for (std::size_t i = base; i < end; ++i)
      m |= static_cast<uint64_t>(populated[i]) << (i - base);
    words[w] = m;
  }
}
